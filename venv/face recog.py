import cv2
import numpy as np

img = cv2.imread("Resources/pic1.jpg")

# creating a matrix of ones of size 5,5 with object unsigned int
kernel = np.ones((5,5),np.uint8)

imgGray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
imgBlur = cv2.GaussianBlur(imgGray,(7,7),0)# object, kernel size (odd no), sigmax
imgCanny = cv2.Canny(img,150,200) # object, Threshold1, threshold 2
imgDilation = cv2.dilate(imgCanny, kernel, iterations=1) # check by increased iteration
imgEroded = cv2.erode(imgDilation, kernel, iterations=1)

cv2.imshow("gray Image",imgGray)
cv2.imshow("Blur Image",imgBlur)
#cv2.imshow("Canny Image",imgCanny)
#cv2.imshow("Dilation Image",imgDilation)
#cv2.imshow("Eroded Image",imgEroded)
cv2.waitKey(0)